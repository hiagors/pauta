...
def test_initiative_name_is_unique_inside_the_project_only(
    ...
    """Uma "Catálogo" no Aurora e outra no Boreal são normais (§6.2)."""
    aurora = world.project("Aurora")
    boreal = world.project("Boreal")
    create = fakes.use_case(CreateInitiative)
    create.execute(CreateInitiativeInput(project_id=aurora.id, name="Catálogo"))

    create.execute(CreateInitiativeInput(project_id=boreal.id, name="Catálogo"))
...
def test_update_initiative_clears_the_layer_with_null(
    ...
    initiative = world.initiative(world.project("Aurora"), "V1", layer="Dados")
...
def test_list_initiatives_filters_and_orders_by_priority(
    ...
    aurora = world.project("Aurora")
    world.initiative(aurora, "Envio", priority=Priority.LOW, layer="Backend")
    world.initiative(aurora, "Catálogo", priority=Priority.HIGH)
    world.initiative(
        aurora, "V2", priority=Priority.MEDIUM, status=InitiativeStatus.CANCELLED
    )
    world.initiative(world.project("Boreal"), "Portal Externo", priority=Priority.HIGH)
    ...
    assert [
        ...
        for view in listing.execute(
            InitiativeFilter(project_id=aurora.id, layer="Backend")
        )
    ] == ["Envio"]
    assert [
        view.name
        for view in listing.execute(
            InitiativeFilter(statuses=(InitiativeStatus.BACKLOG,), query="catál")
        )
    ] == ["Catálogo"]
...
def test_archive_initiative_refuses_when_it_has_allocation(
    ...
    first = world.initiative(project, "V1")
    world.initiative(project, "V2")
    world.allocate(first, 18, squad=world.squad("Alfa"))
