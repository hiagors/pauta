...
def test_a_formula_e_a_do_spec() -> None:
    esperado = hashlib.sha256(f"MEMBER_CONFLICT|{BIANCA}|19".encode()).hexdigest()[:32]
    assert alert_fingerprint(AlertType.MEMBER_CONFLICT, BIANCA, 19) == esperado


def test_tem_trinta_e_dois_hex() -> None:
    fingerprint = alert_fingerprint(AlertType.MEMBER_IDLE, BIANCA, 20)
    assert len(fingerprint) == FINGERPRINT_LENGTH
    assert all(char in "0123456789abcdef" for char in fingerprint)


def test_e_deterministico() -> None:
    primeiro = alert_fingerprint(AlertType.SQUAD_OVERLOADED, DADOS_A, 19)
    segundo = alert_fingerprint(AlertType.SQUAD_OVERLOADED, DADOS_A, 19)
...
def test_muda_com_o_tipo() -> None:
    ...
    ) != alert_fingerprint(AlertType.MEMBER_IDLE, BIANCA, 19)


def test_muda_com_a_sprint() -> None:
    assert alert_fingerprint(
        AlertType.MEMBER_CONFLICT, BIANCA, 19
    ) != alert_fingerprint(AlertType.MEMBER_CONFLICT, BIANCA, 20)


def test_muda_com_o_sujeito() -> None:
    assert alert_fingerprint(
        AlertType.MEMBER_CONFLICT, BIANCA, 19
    ) != alert_fingerprint(AlertType.MEMBER_CONFLICT, uid(11), 19)
...
def test_o_sujeito_e_a_squad_nos_alertas_de_squad() -> None:
    ...
    assert alert_fingerprint(AlertType.EMPTY_SQUAD, DADOS_A, 21) == alert_fingerprint(
        AlertType.EMPTY_SQUAD, DADOS_A, 21
    )
