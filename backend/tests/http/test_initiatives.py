...
def test_creating_an_initiative_stamps_the_entry_date(api: Api) -> None:
    ...
    created = api.initiative(project["id"], "Reestruturação V1", priority="HIGH")
...
def test_the_same_name_in_two_projects_is_allowed(api: Api) -> None:
    ...
    normal."""
    crm = api.project("CRM")["project"]
    bnpl = api.project("BNPL")["project"]

    api.initiative(crm["id"], "Descoberta")
    api.initiative(bnpl["id"], "Descoberta")
...
def test_the_same_name_twice_in_one_project_is_409(api: Api) -> None:
    ...
    response = api.post(
        "/initiatives", json={"project_id": crm["id"], "name": "Descoberta"}
    )
...
def test_the_filters_of_the_spec_narrow_the_list(api: Api) -> None:
    crm = api.project("CRM")["project"]
    bnpl = api.project("BNPL")["project"]
    api.initiative(crm["id"], "Reestruturação V1", priority="HIGH", layer="dados")
    api.initiative(crm["id"], "Descoberta", priority="LOW")
    api.initiative(bnpl["id"], "OpenFinance", priority="HIGH")

    by_project = api.get("/initiatives", params={"project_id": crm["id"]}).json()
    ...
    assert [item["name"] for item in by_layer] == ["Reestruturação V1"]
    assert by_status == []
...
def test_a_patch_clears_the_layer_with_null_and_keeps_it_when_absent(
    ...
    project = api.project("CRM")["project"]
    initiative = api.initiative(project["id"], "Reestruturação V1", layer="dados")
...
def test_deleting_an_initiative_with_an_allocation_is_409(api: Api) -> None:
    api.sprints()
    project = api.project("CRM")["project"]
    initiative = api.initiative(project["id"], "Reestruturação V1")
