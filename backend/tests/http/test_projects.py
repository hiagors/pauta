...
def test_creating_a_project_creates_the_first_initiative(api: Api) -> None:
    ...
    created = api.project("CRM", description="Plataforma de vendas")

    assert created["project"]["name"] == "CRM"
    assert created["project"]["description"] == "Plataforma de vendas"
    assert [item["name"] for item in created["initiatives"]] == ["CRM"]
...
def test_the_list_comes_sorted_by_name(api: Api) -> None:
    for name in ("Portal", "BNPL", "CRM"):
        api.project(name)

    found = api.get("/projects").json()

    assert [item["name"] for item in found] == ["BNPL", "CRM", "Portal"]
...
def test_the_search_folds_case_including_accents(api: Api) -> None:
    ...
    api.project("Reestruturação")
    api.project("BNPL")

    found = api.get("/projects", params={"q": "REESTRUTURAÇÃO"}).json()

    assert [item["name"] for item in found] == ["Reestruturação"]


def test_the_active_filter_separates_the_two_sides(api: Api) -> None:
    kept = api.project("CRM")["project"]
...
def test_get_includes_the_initiatives_of_the_project(api: Api) -> None:
    ...
    api.initiative(project["id"], "Reestruturação V1")
...
def test_a_patch_only_touches_the_fields_it_carries(api: Api) -> None:
    project = api.project("CRM", description="Vendas", color="#0052CC")["project"]

    updated = api.patch(f"/projects/{project['id']}", json={"name": "CRM 2"}).json()

    assert updated["name"] == "CRM 2"
...
def test_a_null_color_clears_the_color(api: Api) -> None:
    ...
    project = api.project("CRM", color="#0052CC")["project"]
...
def test_an_empty_patch_is_a_no_op(api: Api) -> None:
    project = api.project("CRM", description="Vendas")["project"]
