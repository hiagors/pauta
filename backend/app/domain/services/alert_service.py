...
class AlertService:
    ...
    def evaluate(
        ...
        for sprint_number in sorted(snapshot.sprint_numbers):
            alerts.extend(self._squad_overloaded(snapshot, sprint_number))
            alerts.extend(self._member_conflict(snapshot, sprint_number))
            alerts.extend(self._member_idle(snapshot, sprint_number))
            alerts.extend(self._empty_squad(snapshot, sprint_number))
        return [self._apply_mute(alert, mutes) for alert in alerts]
    ...
    def _member_conflict(
        ...
        for member_id, member_name in _sorted(snapshot.members):
            ...
            if len(initiatives) <= _MAX_INITIATIVES_PER_SPRINT:
                continue
            squad_refs = self._squads_behind(
                snapshot, member_id, sprint_number, initiatives
            )
            has_direct = any(
    ...
    def _apply_mute(self, alert: Alert, mutes: Mapping[str, MutedAlert]) -> Alert:
